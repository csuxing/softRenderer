...
namespace RHI
{
    RenderContext::RenderContext(APP::VkDeviceManager* deviceManager) :
        m_deviceManager(deviceManager)
    {
        m_surface = m_deviceManager->getSurface();
        m_device = m_deviceManager->getDevice();
        m_swapchainInfo = m_deviceManager->getSwapchain();
    }

    void RenderContext::init()
    {
        uint32_t imageCount = to_u32(m_swapchainInfo.images.size());
        ...
        for (uint32_t i = 0; i < imageCount; ++i)
        {
            ...
            VK_CHECK(vkCreateFence(m_deviceManager->getDevice(), &info, nullptr, &per_frame.queue_submit_fence));

            VkCommandPoolCreateInfo cmd_pool_info{ VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO };
            cmd_pool_info.flags = VK_COMMAND_POOL_CREATE_TRANSIENT_BIT;
            cmd_pool_info.queueFamilyIndex = m_deviceManager->getGraphicsQueueFamilyIndex();
            VK_CHECK(vkCreateCommandPool(m_deviceManager->getDevice(), &cmd_pool_info, nullptr, &per_frame.primary_command_pool));
            ...
            VK_CHECK(vkAllocateCommandBuffers(m_deviceManager->getDevice(), &cmd_buf_info, &per_frame.primary_command_buffer));
    ...
    void RenderContext::beginFrame()
    {
        ...
        if (m_recycledSemaphores.empty())
        {
            ...
            VK_CHECK(vkCreateSemaphore(m_deviceManager->getDevice(), &info, nullptr, &acquire_semaphore));
        }
        else
        {
        ...
        VkResult res = vkAcquireNextImageKHR(m_deviceManager->getDevice(), m_swapchainInfo.handle, UINT64_MAX, acquire_semaphore, VK_NULL_HANDLE, &m_currentIndex);
        if (res != VK_SUCCESS)
        {
        ...
        {
            vkWaitForFences(m_deviceManager->getDevice(), 1, &m_perframes[m_currentIndex].queue_submit_fence, true, UINT64_MAX);
            vkResetFences(m_deviceManager->getDevice(), 1, &m_perframes[m_currentIndex].queue_submit_fence);
        }

        if (m_perframes[m_currentIndex].primary_command_pool != VK_NULL_HANDLE)
        {
            vkResetCommandPool(m_deviceManager->getDevice(), m_perframes[m_currentIndex].primary_command_pool, 0);
        }
    ...
    void RenderContext::endFrame()
    {
        ...
        if (m_perframes[m_currentIndex].swapchain_release_semaphore == VK_NULL_HANDLE)
        {
            ...
            VK_CHECK(vkCreateSemaphore(m_deviceManager->getDevice(), &semaphore_info, nullptr,
        ...
        VK_CHECK(vkQueueSubmit(m_deviceManager->getGraphicsQueue(), 1, &info, m_perframes[m_currentIndex].queue_submit_fence));

        VkPresentInfoKHR present{ VK_STRUCTURE_TYPE_PRESENT_INFO_KHR };
        present.swapchainCount = 1;
        auto handle = m_swapchainInfo.handle;
        ...
        vkQueuePresentKHR(m_deviceManager->getGraphicsQueue(), &present);
    }

    void RenderContext::initRenderpass()
    {
        VkAttachmentDescription attachment = { 0 };
        attachment.format = m_swapchainInfo.format.format;
        ...
        VK_CHECK(vkCreateRenderPass(m_deviceManager->getDevice(), &rp_info, nullptr, &m_renderpass));
        LOG_DEBUG("renderpass has created!");
    }
    void RenderContext::initPipeline()
    {
        VkPipelineLayoutCreateInfo layout_info{ VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO };
        VK_CHECK(vkCreatePipelineLayout(m_deviceManager->getDevice(), &layout_info, nullptr, &m_pipelineLayout));
        ...
        VK_CHECK(vkCreateGraphicsPipelines(m_deviceManager->getDevice(), VK_NULL_HANDLE, 1, &pipe, nullptr, &m_pipeline));
        LOG_DEBUG("pipeline has created!");
        // Pipeline is baked, we can delete the shader modules now.
        vkDestroyShaderModule(m_deviceManager->getDevice(), shader_stages[0].module, nullptr);
        vkDestroyShaderModule(m_deviceManager->getDevice(), shader_stages[1].module, nullptr);
    }
    void RenderContext::initFrameBuffer()
    {
        VkDevice device = m_device;

        for (auto& image_view : m_swapchainInfo.imageViews)
        {
            ...
            fb_info.width = m_swapchainInfo.extent.width;
            fb_info.height = m_swapchainInfo.extent.height;
            fb_info.layers = 1;
